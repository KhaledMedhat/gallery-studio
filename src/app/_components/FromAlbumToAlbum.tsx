...
const FromAlbumToAlbum: React.FC<{ gallerySlug: string }> = ({
  gallerySlug,
}) => {
  ...
  const { mutate: moveFromAlbumToAlbum, isPending: isMovePending } =
    api.album.moveFromAlbumToAlbum.useMutation({
      onSuccess: () => {
  ...
  const albums = allAlbums?.filter(
    (album) => album.name !== currentAlbum?.name,
  );
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="flex max-w-2xl flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Move selected showcases to another album{" "}
          </DialogDescription>
        </DialogHeader>
